(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .panzoom-floating-head.is-visible { display: block; }
      .panzoom-floating-head-table {
        border-collapse: collapse;
        table-layout: fixed;
        background: transparent;
      }
      .panzoom-floating-head-table thead,
      .panzoom-floating-head-table tr,
      .panzoom-floating-head-table th {
        background: inherit;
      }
      .panzoom-floating-head-table th {
        box-sizing: border-box;
      }
      .panzoom-floating-head-table .col-resize-handle {
  ...
  function setupFloatingHeader(wrapper, table) {
    if (!wrapper || !table || wrapper.dataset.floatingHeaderBound === 'true') return;
    wrapper.dataset.floatingHeaderBound = 'true';

    const floatingHead = document.createElement('div');
    floatingHead.className = 'panzoom-floating-head';
    const floatingTable = document.createElement('table');
    floatingTable.className = `${table.className || ''} panzoom-floating-head-table`.trim();
    floatingHead.appendChild(floatingTable);
    wrapper.insertBefore(floatingHead, wrapper.querySelector(':scope > .panzoom-stage') || wrapper.firstChild);

    function syncFloatingHeader() {
      const thead = table.tHead;
      if (!thead || !thead.rows.length) {
        floatingHead.classList.remove('is-visible');
        floatingTable.innerHTML = '';
        return;
      }

      const sourceCols = Array.from(table.querySelectorAll(':scope > colgroup > col'));
      const colgroup = document.createElement('colgroup');
      sourceCols.forEach((sourceCol) => {
        const cloneCol = document.createElement('col');
        cloneCol.style.width = sourceCol.style.width || `${Math.round(sourceCol.getBoundingClientRect().width)}px`;
        colgroup.appendChild(cloneCol);
      });

      const theadClone = thead.cloneNode(true);
      theadClone.querySelectorAll('.col-resize-handle').forEach((handle) => handle.remove());
      theadClone.querySelectorAll('.col-resize-header').forEach((cell) => cell.classList.remove('col-resize-header'));
      const sourceRows = Array.from(thead.rows);
      const cloneRows = Array.from(theadClone.rows);
      sourceRows.forEach((sourceRow, rowIndex) => {
        const cloneRow = cloneRows[rowIndex];
        if (!cloneRow) return;
        cloneRow.style.background = window.getComputedStyle(sourceRow).backgroundColor;
        Array.from(sourceRow.cells).forEach((sourceCell, cellIndex) => {
          const cloneCell = cloneRow.cells[cellIndex];
          if (!cloneCell) return;
          const cellStyle = window.getComputedStyle(sourceCell);
          cloneCell.style.background = cellStyle.backgroundColor;
          cloneCell.style.color = cellStyle.color;
          cloneCell.style.borderBottom = cellStyle.borderBottom;
          cloneCell.style.borderTop = cellStyle.borderTop;
          cloneCell.style.boxShadow = cellStyle.boxShadow;
          cloneCell.style.opacity = cellStyle.opacity;
          const sourceDirection = sourceCell.dataset ? (sourceCell.dataset.sortDirection || '') : '';
          const cloneArrow = cloneCell.querySelector('.sort-arrow');
          if (cloneArrow) cloneArrow.textContent = sourceDirection === 'asc' ? '↑' : sourceDirection === 'desc' ? '↓' : '';
        });
      });

      floatingTable.innerHTML = '';
      if (colgroup.children.length) floatingTable.appendChild(colgroup);
      floatingTable.appendChild(theadClone);

      const visibleWidth = Math.round(table.getBoundingClientRect().width);
      const unscaledWidth = Math.round(table.offsetWidth || visibleWidth);
      floatingHead.style.width = `${wrapper.clientWidth}px`;
      floatingTable.style.width = `${Math.max(visibleWidth, wrapper.clientWidth)}px`;
      floatingTable.style.transform = `translateX(${-wrapper.scrollLeft}px)`;
      floatingTable.style.transformOrigin = 'top left';

      const shouldShow = wrapper.scrollTop > 0 && visibleWidth > 0 && unscaledWidth > 0;
      floatingHead.classList.toggle('is-visible', shouldShow);
    }

    const scheduleSync = debounceFrame(syncFloatingHeader);
    ...
    window.addEventListener('resize', scheduleSync);
    wrapper.__syncFloatingHeader = syncFloatingHeader;
    syncFloatingHeader();
    return syncFloatingHeader;
  }
  ...
  function setupPanzoom(wrapper) {
    ...
    function applyScale(nextScale, centerX, centerY) {
      ...
      scale = clamped;
      content.style.transform = Math.abs(scale - 1) < 0.001 ? 'none' : `scale(${scale})`;
      ...
      if (syncFloatingHeader) syncFloatingHeader();
    }
    ...
    syncFloatingHeader = setupFloatingHeader(wrapper, table);
    refreshStage();
  }
  ...
  window.refreshPanzoomTableHeaders = function refreshPanzoomTableHeaders() {
    document.querySelectorAll(WRAPPER_SELECTOR).forEach((wrapper) => {
      if (typeof wrapper.__syncFloatingHeader === 'function') wrapper.__syncFloatingHeader();
    });
  };
