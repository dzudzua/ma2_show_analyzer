...
const COLUMN_DEFS = [
  ...
  { key: "dimmerValuesCount", label: "Dimmer Hard Count", className: "cell-tight" },
...
function attributeFamily(attribute) {
  ...
  if (normalized.startsWith("POS") || normalized.startsWith("POSITION") || normalized.startsWith("PAN") || normalized.startsWith("TILT")) return "Position";
  ...
  if (normalized.startsWith("FOCUS") || normalized.startsWith("ZOOM") || normalized.startsWith("IRIS") || normalized.startsWith("BEAM") || normalized.startsWith("PRISM") || normalized.startsWith("FROST")) return "Beam";
  if (normalized.startsWith("CTRL") || normalized.startsWith("CONTROL") || normalized.startsWith("LAMP")) return "Control";
  return "Other";
}

function isDimmerHardAtom(atom) {
  return String(atom?.value_type || "") === "hard" && attributeFamily(atom?.attribute) === "Dimmer";
}
...
function mapSequenceToCueRows(sequence, context) {
  ...
  return (sequence.cues || []).map((cue) => {
    ...
    const dimmerHardAtoms = (cue.values || []).filter((atom) => isDimmerHardAtom(atom));
    const hardAtoms = (cue.values || []).filter((atom) => atom.value_type === "hard" && !isDimmerHardAtom(atom));
    ...
    cueRow.dimmerValuesCount = dimmerHardAtoms.length;
    ...
    cueRow.notes = uniqueStrings([
      ...
      dimmerHardAtoms.length ? `dimmer hard values ${dimmerHardAtoms.length}` : "",
...
function sortCueRows(rows, sortState) {
  ...
  return [...rows].sort((left, right) => {
    ...
    if (["triggerTime", "fade", "delay", "dimmerValuesCount", "hardValuesCount", "fixtureCount"].includes(key)) {
...
function summarizeSequenceRows(rows) {
  ...
  const dimmer = rows.filter((row) => row.dimmerValuesCount > 0).length;
